...
#read in csv in case of api problems
plot_locations<- read.csv("full_locations.csv", na.strings = c("", "NA"))  #read csv file
...
full_map <- NULL
full_map <- ggplot(data=plot_locations, aes(plot_locations$latitude, plot_locations$longitude)) +
  mapWorld +
...
facet_map <- NULL
facet_map <- ggplot(data=plot_locations, aes(x=plot_locations$latitude, y=plot_locations$longitude)) + 
  mapWorld + geom_count(shape = 21, color = "black",
             show.legend = TRUE) + labs(x = "", y = "") + 
  facet_wrap(~elite)
